...
public class Player : BaseGameEntity
{
    ...
    private Dictionary<E_PlayerStates, State<Player>> _states;
    ...
    public override void Setup(string name)
    {
        ...
        _states = new Dictionary<E_PlayerStates, State<Player>>
        {
            {E_PlayerStates.RestAndSleep, new PlayerOwnedStates.RestAndSleep()},
            {E_PlayerStates.FightHard, new PlayerOwnedStates.FightHard()},
            {E_PlayerStates.Global, new PlayerOwnedStates.StateGlobal()}
        };
        
        // 상태를 관리하는 StateMachine에 메모리를 할당하고, 첫 상태를 설정
        _stateMachine = new StateMachine<Player>();
        _stateMachine.Setup(this, _states[E_PlayerStates.RestAndSleep]);
        
        // 전역 상태 설정
        _stateMachine.SetGlobalState(_states[E_PlayerStates.Global]);
        
        
        _health = 0;
        _currentLocation = Locations.Village;
        // 위에처럼 나머지 필드 데이터 셋업
        /*
