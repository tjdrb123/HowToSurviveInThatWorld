...
public class TransitionTableEditor : Editor
{
	...
	{
		Separator();
		HelpBox("상태를 클릭하면 포함된 'Transition'을 볼 수 있습니다. \n(연필/렌치) 아이콘을 클릭하면 해당하는 'Action'을 볼 수 있습니다.", MessageType.Info);
		Separator();
	...
	public void DisplayStateEditor(Object state)
	{
	...
	public void AddTransition(SerializedTransition source)
	{
	...
	public void ReorderTransition(SerializedTransition serializedTransition, bool up)
	{
	...
	public void RemoveTransition(SerializedTransition serializedTransition)
	{
		int stateIndex = _fromStates.IndexOf(serializedTransition.FromState.objectReferenceValue);
		var stateTransitions = _transitionsByFromStates[stateIndex];
	...
	public List<SerializedTransition> GetStateTransitions(Object state)
	{
	...
	private void GroupByFromState()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				DebugLogger.LogError("Transition with invalid \"From State\" found in table " + serializedObject.targetObject.name + ", deleting...");
			...
			{
				DebugLogger.LogError("Transition with invalid \"Target State\" found in table " + serializedObject.targetObject.name + ", deleting...");
				_transitions.DeleteArrayElementAtIndex(i);
				ApplyModifications("Invalid transition deleted");
				return;
			}
